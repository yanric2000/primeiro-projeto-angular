...
@Component({
    ...
    styleUrls: ['./new-game.component.styl']
})
export class NewGameComponent implements OnInit {
    ...
    async ngOnInit() {
        await this.platformsService.getAllPlatforms().subscribe(
            data => {
                ...
                const options = this.allPlatforms;

                // Adiciono o valor "available" em todas as plataformas
                ...
                this.game.platformEnable = options;
            }
        );
    ...
    checkIfExist(array: Array<any>, value: any, property: string){
        if(property) {
            return array.find(item => item[property] === value) ? true : false;
        }
    ...
    filterGamePlatforms() {
        this.game.platformEnable = this.game.platformEnable.filter(platform => platform.available === true);
        this.game.platformEnable.map(platform => {
            delete platform.available;
        });
    }
    ...
    addGame(event) {
        ...
        this.showLoading();
        this.game.dateCreated = formattedDate;
        this.filterGamePlatforms();
        
        this.gameServices.addGame(this.game).subscribe(data => { 
            this.callbackResponse = 'Jogo adicionado com sucesso!';
            this.hiddeLoading();
